...
use Exception;
use Sandeepchowdary7\Laratriposo\Interfaces\TriposoInterface;
...
Class Triposo implements TriposoInterface
{
    ...
    */
    public function __construct($api_token = null,$account_id = null) {
        if (!$api_token) {
            $api_token = config('triposo.api_token', 'api_token');
        }
        if (!$account_id) {
            $account_id = config('triposo.account_id', 'account_id');
        }
        $api_token = trim($api_token);
        $account_id = trim($account_id);

        if (empty($api_token) || !preg_match('#^[\w-]+$#si', $api_token)) {
            throw new Exception("Missing or invalid Triposo API token.");
        }
        if (empty($account_id)) {
            throw new Exception("Missing or invalid Triposo Account ID.");
        }
